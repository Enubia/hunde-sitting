...
import type { BookingStatus, LocationType, ServiceType, VaccinationStatus } from '../schema/db';
...
export async function seed(db: Kysely<DB>): Promise<void> {
    ...
    for (let i = 0; i < 20; i++) {
        ...
        if (Math.random() > 0.5) {
            ...
            const serviceTypes = ['dog_walking', 'overnight_stay', 'daycare', 'boarding', 'training', 'grooming', 'vet_visits', 'pet_taxi'] as const;
            ...
            for (let j = 0; j < numServices; j++) {
                await db
                    ...
                    .values({
                        sitter_id: sitter.id,
                        service_name: shuffledServices[j] as ServiceType,
        ...
        for (let i = 0; i < numDogs; i++) {
            ...
            const [dog] = await db
                ...
                .values({
                    ...
                    vaccination_status: ['fully_vaccinated', 'partially_vaccinated', 'not_vaccinated', 'unknown'][Math.floor(Math.random() * 4)] as VaccinationStatus,
    ...
    // const bookingStatuses = ['pending', 'confirmed', 'in_progress', 'completed', 'cancelled', 'rejected'];
    ...
    for (let i = 0; i < 40; i++) {
        ...
        const [booking] = await db
            ...
            .values({
                ...
                location_type: locationTypes[Math.floor(Math.random() * locationTypes.length)] as LocationType,
    ...
    // Insert user groups if they don't exist yet
    const groups = await db.selectFrom('user_groups').select('name').execute();
    ...
    // Create user groups
        await db
            ...
            .values([
                ...
                { name: 'moderators', description: 'Can review and moderate content' },
                { name: 'sitters', description: 'Dog sitters with access to their own data and bookings' },
                { name: 'clients', description: 'Pet owners with access to their own data and bookings' },
            ])
            .execute();
    }
